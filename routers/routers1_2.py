from fastapi import APIRouter, HTTPException, status #Importamos el framework fastapi a nuestro entorno de trabajo
...
@router.get("/usersclass/{id}",status_code=status.HTTP_200_OK)
...
@router.post("/usersclass/", response_model=User, status_code=status.HTTP_201_CREATED)
...
@router.put("/usersclass/", response_model=User, status_code=status.HTTP_201_CREATED)
async def usersclass(user:User):
    ...
    #http://127.0.0.1:8000/usersclass/
        ...
        #***Delete
@router.delete("/usersclass/{id}", status_code=status.HTTP_204_NO_CONTENT) #Aquí no es necesario poner todo el usuario, con el id basta para eoncontrarlo y eliminarlo
async def usersclass(id:int):
